...
const AdminPanel = () => {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        setForm({ id: form.id.trim(), status: data.status || "Pending", customerName: data.customerName || "", origin: data.origin || "", destination: data.destination || "", eta: data.eta || "", notes: data.notes || "" });
        setMessage("Loaded existing shipment.");
      } else { setMessage("No shipment found. You can create a new one."); }
    } catch (err) { setMessage("Error loading shipment."); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const docRef = doc(db, "shipments", form.id.trim());
      await setDoc(docRef, { status: form.status, customerName: form.customerName, origin: form.origin, destination: form.destination, eta: form.eta, notes: form.notes, updatedAt: serverTimestamp() }, { merge: true });
  ...
  const handleLogin = async (e) => {
    ...
    catch (err) { setLoginError("Login failed. Check your credentials."); }
  ...
  const handleProductSave = async (e) => {
    ...
    try {
      ...
      for (const file of productImages) {
        const fd = new FormData(); fd.append("file", file); fd.append("upload_preset", "oiwrpbwq");
        const res = await fetch("https://api.cloudinary.com/v1_1/dmynksk5z/auto/upload", { method: "POST", body: fd });
        const data = await res.json(); if (data.secure_url) imageUrls.push(data.secure_url);
      }
      let videoUrl = "";
      if (productVideo) {
        const fd = new FormData(); fd.append("file", productVideo); fd.append("upload_preset", "oiwrpbwq");
        const res = await fetch("https://api.cloudinary.com/v1_1/dmynksk5z/auto/upload", { method: "POST", body: fd });
        const data = await res.json(); if (data.secure_url) videoUrl = data.secure_url;
      }
      ...
      const snap = await getDocs(collection(db, "products"));
      setProducts(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    } catch (err) { setProductSaveMessage("Save failed. Please try again."); }
    finally { setProductLoading(false); }
  };

  const handleDeleteProduct = async (id) => {
    try { await deleteDoc(doc(db, "products", id)); setProducts((prev) => prev.filter((p) => p.id !== id)); }
    catch (err) { setProductSaveMessage("تعذر حذف المنتج"); }
  };
  ...
  const handleClientSave = async (e) => {
    ...
    try {
      ...
      setClientMessage("تم حفظ العميل بنجاح"); setClientForm({ name: "" }); setClientFile(null); setClientFileKey((prev) => prev + 1);
      const snap = await getDocs(collection(db, "clients"));
      setClients(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    } catch (err) { setClientMessage("تعذر حفظ العميل. حاول مرة أخرى."); }
    finally { setClientLoading(false); }
  };

  const handleDeleteClient = async (id) => {
    try { await deleteDoc(doc(db, "clients", id)); setClients((prev) => prev.filter((c) => c.id !== id)); }
    catch (err) { setClientMessage("تعذر حذف العميل"); }
  };
  ...
  const handleNewsSave = async (e) => {
    ...
    } catch (err) { setNewsMessage("تعذر حفظ الخبر"); }
    finally { setNewsLoading(false); }
  };

  const handleDeleteNews = async (id) => {
    try { await deleteDoc(doc(db, "news", id)); setNewsItems((prev) => prev.filter((n) => n.id !== id)); }
    catch (err) { setNewsMessage("تعذر حذف الخبر"); }
  };

  const handleMarkRead = async (id) => {
    try { await updateDoc(doc(db, "messages", id), { status: "Read" }); setMessages((prev) => prev.map((msg) => (msg.id === id ? { ...msg, status: "Read" } : msg))); }
    catch (err) { /* ignore */ }
  };

  // ── ALL RENDER FUNCTIONS AFTER HANDLERS ──
      ...
      <div className="card-body p-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <div className="d-flex gap-2">
            <button type="button" className="btn btn-outline-secondary btn-sm" onClick={handleReset} disabled={loading}>مسح الحقول / Reset</button>
            <button type="button" className="btn btn-outline-primary btn-sm" onClick={handleLoad} disabled={loading}>تحميل / Load</button>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="row g-3">
          <div className="col-md-6"><label className="form-label">Tracking ID</label><input name="id" type="text" className="form-control" value={form.id} onChange={handleChange} placeholder="مثال: QIM-123456" /></div>
    ...
    <div className="card shadow-sm border-0">
      <div className="card-body p-4">
        ...
        <form className="row g-3" onSubmit={handleProductSave}>
          ...
          <div className="col-12"><button type="submit" className="btn btn-primary w-100" style={{ background: "var(--secondary-color)", border: "none" }} disabled={productLoading}>{productLoading ? "جارٍ الحفظ..." : "حفظ المنتج / Save Product"}</button></div>
        </form>
      </div>
    </div>
  );
    ...
    <div className="card shadow-sm border-0">
      <div className="card-body p-4">
        ...
        <form className="row g-3" onSubmit={handleClientSave}>
          <div className="col-md-6"><label className="form-label">اسم العميل / Client Name</label><input name="name" type="text" className="form-control" value={clientForm.name} onChange={handleClientChange} required /></div>
          <div className="col-md-6"><label className="form-label">شعار العميل / Client Logo</label><input key={clientFileKey} name="clientLogo" type="file" accept="image/*" className="form-control" onChange={handleClientChange} required /></div>
          <div className="col-12"><button type="submit" className="btn btn-primary w-100" style={{ background: "var(--secondary-color)", border: "none" }} disabled={clientLoading}>{clientLoading ? "جارٍ الحفظ..." : "حفظ العميل / Save Client"}</button></div>
        </form>
      </div>
    </div>
  );
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-striped align-middle">
              <thead className="table-dark">
                <tr><th>الاسم</th><th>رقم الهاتف</th><th>نوع الطلب</th><th>الرسالة</th><th>الحالة</th><th></th></tr>
              </thead>
              <tbody>
                {messages.map((msg) => (
                  <tr key={msg.id}>
  ...
  if (!user) {
    return (
      <main id="main" className="py-5">
        <Helmet><title>Admin Login | Qimmah Track & Trace</title></Helmet>
              ...
              <div className="card shadow-sm border-0">
                <div className="card-body p-4">
                  ...
                  <form onSubmit={handleLogin} className="row g-3">
                    ...
                    <div className="col-12"><button type="submit" className="btn btn-primary w-100" style={{ background: "var(--secondary-color)", border: "none" }} disabled={loginLoading}>{loginLoading ? "جارٍ الدخول..." : "دخول / Login"}</button></div>
                  </form>
                </div>
              </div>
            </div>
  ...
  return (
    <main id="main" className="py-5">
      <Helmet>
        ...
        <meta name="description" content="Manage shipments for Qimmah: create, update, and track statuses." />
      </Helmet>
      <div className="container" data-aos="fade-up">
        <div className="row">
          ...
          <div className="col-lg-9">
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                <button className="btn btn-outline-danger btn-sm" onClick={handleLogout}>خروج / Logout</button>
              </div>
            </div>
